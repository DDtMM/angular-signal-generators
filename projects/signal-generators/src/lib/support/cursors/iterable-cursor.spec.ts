import { ERR_BACKWARD_MOVE, ERR_UNABLE_TO_RESET, IterableCursor } from './iterable-cursor';

describe('IterableCursor', () => {
  it('#moveTo() will move cursor to the next matching element', () => {
    const sut = new IterableCursor([1, 2, 3, 2, 5], false);
  ...
  it('#moveTo() will search from start if matching element is not found and autoReset is on', () => {
    const sut = new IterableCursor([1, 2, 3, 2, 5], true);
    expect(sut.moveTo(5)).toEqual({ hasValue: true, value: 5 });
    expect(sut.moveTo(2)).toEqual({ hasValue: true, value: 2 });
    expect(sut.next()).toEqual({ hasValue: true, value: 3 });
  });
  it('#moveTo() will not search past maxSearchSize', () => {
    const sut = new IterableCursor([1, 2, 3, 4, 5], true, 3);
    expect(sut.moveTo(4)).toEqual({ hasValue: false });
  });
  ...
  it('#next() returns empty result when source is empty and autoReset is on', () => {
    const sut = new IterableCursor([], true);
    expect(sut.next().hasValue).toEqual(false);
  });
  ...
  it('#next(-N) throw', () => {
    const sut = new IterableCursor([1, 2, 3], false);
    expect(() => sut.next(-1)).toThrowError(ERR_BACKWARD_MOVE);
  });
  ...
  it('#next(N = some value much greater that Array.length) returns result.', () => {
    const sut = new IterableCursor([1, 2, 3], true);
  ...
  it('#reset() throws when underlying iterable cannot be reset', () => {
    const sut = new IterableCursor(generatorFromArray([1, 2, 3]), false);
    expect(() => sut.reset()).toThrowError(ERR_UNABLE_TO_RESET);
  });
});
